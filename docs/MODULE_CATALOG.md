...
| Exploration | fixed-camera playable world loop       | characters, map        |
| Roads       | terrain-aware route hierarchy          | map, settlements       |
| Weather     | deterministic local weather fronts     | map, clock             |
| Settlements | place and grow one settlement          | map, characters, roads |
| Economy     | resources, inventories, local exchange | settlements            |
| Combat      | data-driven encounters and equipment   | characters             |
| Vehicles    | roads, steering, traversal modes       | map, characters, roads |
