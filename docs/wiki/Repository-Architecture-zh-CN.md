...
流式世界中的未知采样不会被猜成海岸线。在传输/回退、跨驱逐重新加载接缝、基准测试及
浏览器视觉门槛通过前，该候选不会加入 Worker 默认能力列表；TypeScript 轮廓追踪仍是权威。
...
IndexedDB 保存版本化快照、探索状态、角色位置、拓扑别名、持久化实体消失状态、内容包
解析信息与命名槽位。覆盖槽位时会保留有界历史；每条记录独立校验，恢复必须由用户明确
触发。预览只读取紧凑元数据，不读取渲染缓冲。没有账号、遥测、远程存档或联网玩法。
